...
export function randomSelect(min, max, totalNumbers) { // min and max included 
    const RandomNumbers = []
    while(RandomNumbers.length < totalNumbers/2 ){
...
export async function getShuffeledArrayOfCards(randomNumbers, total_cards) {
    ...
    while(i < count){
        ...
        cards.push({ id, title: dataOfSingleCard.first.title, url:dataOfSingleCard.first.url })
        cards.push({ id, title: dataOfSingleCard.second.title, url:dataOfSingleCard.second.url })
        i++;
    }
...
export function generateCard(card){
    ...
    <div class="my-flip-card w-[100px] h-[150px]">
        ...
        <div title="${card.id}" class="flip-it my-backface-hidden absolute w-full h-full"></div>
        <div class="p-1 rounded my-transform-y-180 my-bg-x my-backface-hidden absolute w-full h-full">
            <div class="flex gap-2 flex-col justify-center items-center bg-white rounded w-full h-full">
